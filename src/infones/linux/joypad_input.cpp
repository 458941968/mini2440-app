...
static int USBjoypadGet(void)
{
	...
	{
		printf("type:0x%x value:0x%x number:0x%x\n", e.type, e.value, e.number);
		if(0x2 == e.type)
		{
			/*
			上：
			...
			*/
			...
			value:0x0 type:0x2 number:0x5
			*/
			if(0x7fff == e.value && 0x7 == e.number)
			{
				joypad &= ~(1<<5);
				joypad |= 1<<5;
			}
			...
			*/
			if(0x8001 == e.value && 0x6 == e.number)
			{
				joypad &= ~(1<<6);
				joypad |= 1<<6;
			}
			...
			value:0x0 type:0x2 number:0x4
			*/
			if(0x7fff == e.value && 0x6 == e.number)
			{
				joypad &= ~(1<<7);
				joypad |= 1<<7;
			}
