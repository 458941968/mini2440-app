...
{
    grayFrameBuff = (unsigned short *)malloc(sizeof(WorkFrame));
...
void NesScreenWidget::loadFrame()
{
    ...
    else
    {
        ...
        {
            unsigned short color = WorkFrame[i];
            grayFrameBuff[i] = (color>>3)<<4|(color&0x001f);
        }
        image = QImage((uchar *)grayFrameBuff, NES_DISP_WIDTH, NES_DISP_HEIGHT, QImage::Format_RGB16);
    }
